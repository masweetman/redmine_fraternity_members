class FraternityMembersController < ApplicationController
  ...
  def index
    sort = params[:sort]
    case sort
    when 'firstname' then @fraternity_members = FraternityMember.where.not(firstname: '').sort_by{|a| [a.firstname.downcase]}
    when 'lastname' then @fraternity_members = FraternityMember.where.not(lastname: '').sort_by{|a| [a.lastname.downcase]}
    when 'pledge_name' then @fraternity_members = FraternityMember.where.not(pledge_name: '').sort_by{|a| [a.pledge_name.downcase]}
    when 'zip' then @fraternity_members = FraternityMember.where.not(address: '').sort_by{|a| [a.address]}
